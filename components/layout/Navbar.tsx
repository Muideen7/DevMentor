...
export default function Navbar() {
  ...
  // When on the landing page use anchor links; from any other page prefix with /
  const navLinks = [
    ...
    <>
      <nav className="sticky top-0 z-50 bg-background-light/80 backdrop-blur-md border-b border-primary/5 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          {/* Logo component already handles its own routing to home */}
          <Logo />
      ...
      >
        <div className="flex flex-col h-full pt-24 px-8 space-y-8">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              className="text-2xl font-bold text-primary active:text-accent-coral transition-all duration-500"
              href={link.href}
              style={{
