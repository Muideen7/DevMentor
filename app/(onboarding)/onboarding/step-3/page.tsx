  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const profileResponse = await fetch("/api/onboarding/complete", {
        ...
        body: JSON.stringify({ goal, currentLevel, stack, hoursPerWeek }),
      })
      ...
      // 2. Generate roadmap via Gemini — must succeed before anything else
      ...
      // 3. Only update session after both profile save and roadmap generation succeed
      await update({
        user: {
          ...session?.user,
    ...
    <>
      <div className="text-center mb-10">
        ...
        <p className="text-slate-500 text-lg leading-relaxed max-w-[540px] mx-auto">
          There&apos;s no wrong answer. Your roadmap adjusts to what&apos;s realistic
          for your life, not what looks impressive on paper.
        </p>
      </div>
      ...
      <div className="w-full space-y-4 mb-10">
        {options.map((opt) => (
          <label key={opt.id} className="cursor-pointer block">
            ...
            <div
              className={cn(
                "flex items-start gap-4 p-5 border-2 rounded-2xl bg-white transition-all hover:border-slate-300",
                hoursPerWeek === opt.hours
                  ? "border-accent-coral bg-coral-tint"
                  : "border-slate-100"
              )}
            >
              {/* Radio circle — fixed with JS condition instead of broken peer-checked */}
              <div
                className={cn(
                  "w-6 h-6 rounded-full border-2 mt-1 flex items-center justify-center transition-colors shrink-0",
                  hoursPerWeek === opt.hours
                    ? "border-accent-coral"
                    : "border-slate-200"
                )}
              >
                <div
                  className={cn(
                    "w-3 h-3 rounded-full bg-accent-coral transition-opacity",
                    hoursPerWeek === opt.hours ? "opacity-100" : "opacity-0"
                  )}
                />
              </div>
      ...
      <div className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-8 mb-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
        <h3 className="text-slate-400 text-xs font-bold mb-6 uppercase tracking-widest">
          Based on your selections...
        </h3>
        <ul className="space-y-4">
          <li className="flex items-center gap-3">
            <div className="w-5 h-5 bg-accent-coral/20 rounded-full flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-[12px] text-accent-coral font-bold">
                check
              </span>
            </div>
            <span className="text-slate-700 font-medium">
              Goal:{" "}
              <span className="text-slate-900 font-bold underline decoration-accent-coral/30">
                {goal || "Build Web Apps"}
              </span>
            </span>
          </li>
          <li className="flex items-center gap-3">
            ...
            <span className="text-slate-700 font-medium">
              Starting at{" "}
              <span className="text-slate-900 font-bold underline decoration-accent-coral/30">
                {currentLevel || "the basics"}
              </span>{" "}
              with{" "}
              <span className="text-slate-900 font-bold underline decoration-accent-coral/30">
                {stack.join(", ") || "various technologies"}
              </span>
            </span>
          </li>
          <li className="flex items-center gap-3">
      ...
      <div className="w-full flex flex-col items-center gap-6 pb-12">
        <button
          ...
          className="w-full bg-primary text-white py-5 rounded-full text-lg font-bold flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? (
            ...
            <>
              Generate My Roadmap
              <span className="material-symbols-outlined text-accent-coral font-bold">
                auto_awesome
              </span>
            </>
          )}
